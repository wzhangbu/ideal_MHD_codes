...
__e = 1.6e-19  # Elementary charge in Coulombs
...
class Data:
    ...
    @classmethod
    def _read_from_file_ideal(cls, Data_obj, path, axis1range, axis2range, xline=1, SM=0, file=None):
        ...
        names = list(name.lower().replace("_", "") for name in data.dtype.names)
    ...
    @classmethod
    def _read_from_file_EMIC(cls, Data_obj, path, axis1range, axis2range, s=25, scaling=16, xline=1):
        ...
        for i in range(3):
            ...
            J[:, i] = (rho_i * Ui[:, i] - rho_e * Ue[:, i] * s) * 1e15 * cls.__e / scaling
